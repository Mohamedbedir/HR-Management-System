...
namespace HR.Core.Features.Departments.Commands.Validators
{
    public class EditDepartmentValidator:AbstractValidator<EditDepartmentCommand>
    {
        ...
        {
            RuleFor(s => s.Name)
                .MustAsync(async (model,key, cancellationToken) => !await departmentService.IsStudentExistExcludeSelf(key,model.Id))
